...
const CountriesForm = () => {
  ...
  useEffect(() => {
    ...
    if (id) {
      ...
      setMode(
        path.includes("toolsGeographicalContentCountriesView")
  ...
  const fetchCountry = async () => {
    try {
      const response = await apiHandler.get(`/api/countries/${id}`);
      ...
      if ((httpOk || apiOk) && c) {
        ...
        setTimeout(() => {
          Object.keys(countryData).forEach((k) =>
            setValue(k, countryData[k], { shouldValidate: true })
          );
          trigger();
          Object.keys(countryData).forEach((k) => clearErrors(k));
        }, 50);
      } else {
        toast.error(response?.data?.message || "Failed to fetch country");
      }
    } catch (err) {
      const msg =
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response =
        mode === "create"
          ? await apiHandler.post("/api/countries", payload)
          : await apiHandler.patch(`/api/countries/${id}`, payload);
      ...
      if (response?.status) {
        ...
        navigate(
          Constants.URLConstants.TOOLSGEOGRAPHICALCONTENTCOUNTRIESSEARCH
        );
      } else {
        toast.error(
          response?.message ||
            `Failed to ${mode === "create" ? "create" : "update"} country`
        );
      }
    } catch (err) {
      const msg =
  ...
  const title =
    mode === "toolsGeographicalContentCountriesView"
      ? "VIEW COUNTRY"
      : mode === "toolsGeographicalContentCountriesEdit"
      ? "EDIT COUNTRY"
      : "Add Country";
  ...
  return (
    <>
      <Header2
        ...
        linkText2={title}
        link1={Constants.URLConstants.TOOLSGEOGRAPHICALCONTENTCOUNTRIESSEARCH}
      />
          ...
          <div className="row mt-2">
            <div className="col-md-3 form-group">
              ...
              <input
                ...
                readOnly={mode === "toolsGeographicalContentCountriesView"}
                disabled={mode === "toolsGeographicalContentCountriesView"}
                maxLength={300}
          ...
          {mode !== "toolsGeographicalContentCountriesView" ? (
            ...
            <div className="d-flex justify-content-end align-items-end mt-4 gap-2">
              <button
                ...
                onClick={() =>
                  navigate(`/toolsGeographicalContentCountriesEdit/${id}`)
                }
              >
