...
import { ErrorApiAlert, RequiredFieldAlert, SuccessApiToast, filterOptionsByLabel, formatDate } from "../../../constants/globalfunctions";
import excelfilereader from "../../../constants/excelfilereader";
import { taxregpaymentmode } from "../../../constants/contants";
...
const OfflineSuppliersTaxRegistration = ({ data }) => {
    ...
    useEffect(() => {
        if (data && Object.keys(data).length > 0) {
           ...
           setFormData({
               ...
               payment_mode: taxData.payment_mode ?  taxData.payment_mode  : null,
               bank_name: taxData.bank_name || '',
               swift_code: taxData.swift_code || '',
               acc_type: taxData.acc_type || '',
               acc_name: taxData.acc_name || '',
               acc_no: taxData.acc_no || '',
    ...
    const handlePaymentModeChange = (selectedOption) => {
        setFormData({ ...formData, payment_mode: selectedOption });
    };
    ...
    const handleSubmit = async(e) => {
        ...
        formData.payment_mode=( formData.payment_mode &&  formData.payment_mode.value) || formData.payment_mode;
       const requesbody={
        ...
        if (isSuccessfull) {
         try {
             ...
             const response = await putDATA(ApiRoutes.SUPPLIERS.OFFLINE.TAX_REGISTRATION, data.uuid, requesbody);
                ...
                />
                        ...
                        <div className="row mt-3">
                            <div className="form-group col-md-3">
                                ...
                                <MultiSelect
                                    options={taxregpaymentmode}
                                    isSearchable
                                    placeholder="- Select -"
                                    value={taxregpaymentmode.find(option => option.value === formData.payment_mode)}
                            ...
                            <div className="form-group col-md-3">
                                <label>Bank :</label>
                                <input className="form-control form-control-sm" type="text" name="bank_name" id="bank" value={formData.bank_name} onChange={handleInputChange} />
                            </div>
                            <div className="form-group col-md-3">
                                <label>Swift Code :</label>
                                <input className="form-control form-control-sm" type="text" name="swift_code" id="bank" value={formData.swift_code} onChange={handleInputChange} />
                            </div>
                        </div>
                        <div className="row mt-3">
                            <div className="form-group col-md-3">
                                <label>Account Type :</label>
                                <input className="form-control form-control-sm" type="text" name="acc_type" id="bank" value={formData.acc_type} onChange={handleInputChange} />
                            </div>
                            <div className="form-group col-md-3">
                                <label>Account Name :</label>
                                <input className="form-control form-control-sm" type="text" name="acc_name" id="bank" value={formData.acc_name} onChange={handleInputChange} />
                            </div>
                            <div className="form-group col-md-3">
                                <label>Account Number :</label>
                                <input className="form-control form-control-sm" type="text" name="acc_no" id="bank" value={formData.acc_no} onChange={handleInputChange} />
                            </div>
                        </div>
                        <div className="row mt-3">
                            <div className="form-group col-md-3">
...
const mapStateToProps = (state) => ({
  ...
  export default connect(mapStateToProps)(OfflineSuppliersTaxRegistration);