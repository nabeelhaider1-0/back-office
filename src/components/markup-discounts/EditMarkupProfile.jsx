...
const EditMarkupProfile = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const { formData, setFormData, rawRules, loading, error } = useProfileData('markup', id);
  console.log('EditMarkupProfile - formData:', formData);
  ...
  const {
    ...
    removeRule,
    handleSubmit,
  } = useProfileLogic(formData, setFormData, parameters, 'markup', true, id, navigate);
  ...
  return (
    <>
      <Header2 title="EDIT MARKUP PROFILE" linkText1="Markup Profiles" linkText2="Edit Profile" />
      <ProfileForm
        formData={formData}
...
export default EditMarkupProfile;