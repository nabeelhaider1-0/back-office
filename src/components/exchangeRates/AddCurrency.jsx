...
import { createCurrency } from "./CurrencyService";
...
const AddCurrency = () => {
  ...
  const validateCurrencyCode = (value) => {
    ...
    // Common ISO 4217 validation - you can expand this list or use an API
  ...
  const validationSchema = {
    ...
    is_base_currency: {
      validate: (value) => {
        // Base currency validation can be handled in backend
        return true;
      },
    },
    is_active: {
  ...
  // Helper function to check if only one base currency exists (client-side warning)
  const baseCurrencyWarning = () => {
    // This would typically be checked on the backend, but we can show a warning
    if (formValues.is_base_currency) {
      return (
        <small className="text-warning2 d-block">
          <i className="fa fa-exclamation-triangle triangularFA"></i>
          Only one currency can be the base currency. This will replace the
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const result = await createCurrency(cleanedData);
      if (result.success) {
      ...
      const errorMessage = error.message || "Failed to add currency";
              ...
              <div className="form-group col-md-4">
                ...
                <input
                  ...
                  {...register("currency_code", {
                    ...validationSchema.currency_code,
                    onChange: (e) => {
                      // Force uppercase for currency codes
                      const upperValue = e.target.value.toUpperCase();
                      setValue("currency_code", upperValue, {
                        shouldValidate: true,
                      });
                    },
                  })}
