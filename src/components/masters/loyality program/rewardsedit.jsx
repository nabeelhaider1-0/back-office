...
const MastersLoyalityProgramRewardsEdit = ({data}) => {
  ...
  useEffect(() => {
    if (data && Object.keys(data).length > 0) {
      ...
      setFormData({
        ...
        productCategoryUuid: data.productCategoryUuid.uuid,
        tierUuid: data.tierUuid.uuid
      });
    ...
    formData.productImage=resp.imagelink;
      ...
      try {
        const response = await putDATA(
          ApiRoutes.LOYALTY_PROGRAM.REWARDS,data.uuid,
          formData
        );
     
        if (response.data.statusCode === 200) {
          SuccessApiToast("Loyalty Product Updated Successfully");

          navigate(Constants.URLConstants.MASTERSLOYALITYPROGRAMREWARDSSEARCH);
        }
      } catch (error) {
        ErrorApiAlert("Error Updating Loyalty Product");
      }
  ...
  return (
    <>
      ...
      <div class="container-fluid pt-0 p-4" id="content-pad">
        <form onSubmit={handleSubmit} id="loyaltyproductedit">
...
export default connect(mapStateToProps)(MastersLoyalityProgramRewardsEdit);