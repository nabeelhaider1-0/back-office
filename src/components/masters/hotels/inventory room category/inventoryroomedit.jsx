...
const MastersHotelInventoryRoomCategoryEdit = ({ data }) => {
  ...
  useEffect(() => {
    if (data && Object.keys(data).length > 0) {
      setFormData({
        inventoryRoomCategory: data.inventoryRoomCategory,
      });
    } else {
    ...
    if (isSuccessfull) {
      try {
        ...
        if (response.data.statusCode === 200) {
          SuccessApiToast("Inventory Room Category Updated Successfully");

          navigate(Constants.URLConstants.MASTERSINVENTORYROOMCATEGORYSEARCH);
        }
      } catch (error) {
        ErrorApiAlert("Error Updating Inventory Room Category");
      }
    }
  };
  ...
  return (
    <>
      <Header2
        title="EDIT INVENTORY ROOM DETAILS"
...
export default connect(mapStateToProps)(MastersHotelInventoryRoomCategoryEdit);
