...
const MastersHotelRoomViewEdit = ({ data }) => {
  ...
  useEffect(() => {
    if (data && Object.keys(data).length > 0) {
      setFormData({
        viewBasis: data.viewBasis,
      });
    } else {
      // If data is not available, navigate to the branch search page
      navigateOnRefresh(Constants.URLConstants.MASTERSROOMVIEWSEARCH);
    }
    ...
    if (isSuccessfull) {
      try {
        ...
        if (response.data.statusCode === 200) {
          SuccessApiToast("ViewBasis Updated Successfully");

          navigate(Constants.URLConstants.MASTERSROOMVIEWSEARCH);
        }
      } catch (error) {
        ErrorApiAlert("Error Updating View Basis");
      }
    }
  };
...
export default connect(mapStateToProps)(MastersHotelRoomViewEdit);
