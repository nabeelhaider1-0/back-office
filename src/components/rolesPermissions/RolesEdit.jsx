...
const RolesEdit = () => {
  ...
  useEffect(() => {
    ...
    const fetchRole = async () => {
      try {
        const response = await apiHandler.get(`/admin/roles/${id}`);
        if (response.success) {
          const roleData = {
            name: response.data.role.name || "",
            status: response.data.role.status || "active",
          };
          ...
          setTimeout(() => {
            Object.keys(roleData).forEach((field) => {
              setValue(field, roleData[field], { shouldValidate: true });
            });
            ...
            // Set rights separately since they're managed via checkboxes
            const rights = response.data.roleRights.map((rr) => ({
              id: rr.id,
              allowed_actions: rr.allowed_actions,
            }));

            // Set rights using setValue for form tracking
            setValue("rights", rights, { shouldValidate: true });
          }, 100);
        } else {
          toast.error(response.message || "Failed to fetch role");
        }
      } catch (error) {
        const errorMessage =
  ...
  const onSubmit = async (data) => {
    ...
    // Validate rights before submission
    const rightsValidation = validateRights(cleanedData.rights);
    if (rightsValidation !== true) {
      toast.error(rightsValidation);
      setIsSubmitting(false);
      return;
    }

    try {
      const response = await apiHandler.put(`/admin/roles/${id}`, cleanedData);
      if (response.success) {
