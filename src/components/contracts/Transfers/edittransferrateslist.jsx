...
const ContractsEditTransfersRateslist = ({ data }) => {
  ...
  const redirect = async () => {
    navigateOnRefresh(
      Constants.URLConstants.CONTRACTSTRANSFERSRATESSEARCHTRANFERLIST
    );
  };
  ...
  function getModifiedVehicleTypeTransfer(selectedVehicleUuid) {
    const vehicleTypeTransfers = data.vehicleTypeTransfers;
    const matchingRecord = vehicleTypeTransfers.find(
      (record) => record.uuid === selectedVehicleUuid
    );
    ...
    const modifiedRecord = { ...matchingRecord };

    return modifiedRecord;
  }
  function datePassObject(value) {
    var DateArray = value.split("/"); // Splitting the string into an array ["04", "08", "2024"]
    var DateObject = new Date(DateArray[2], DateArray[0] - 1, DateArray[1]); // Creating a Date object
    return DateObject;
  }
  useEffect(() => {
    if (data && Object.keys(data).length > 0) {
      setStartDate(datePassObject(data.periodFrom));
      setEndDate(datePassObject(data.periodTo));

      const result = getModifiedVehicleTypeTransfer(data.selectedvehicleuuid);

      if (!loading && vehicleOptions.length > 0 && data) {
        const defaultVehicle = vehicleOptions.find(
          (option) => option.value === result.vehicle[0].uuid
        );
          ...
          const updatedRows = rows.map((row) =>
            row.id === 1
              ? {
                  ...
                  chargeType: result.chargeAppliedType,
                }
              : row
          );
          setRows(updatedRows);
          // setRows([{ id: 1, chargeType: 'Private', selectedVehicle: result.vehicle[0].uuid }]);
        } else {
          // console.warn('Default vehicle not found in options');
        }
      }
      if (!loadingtrans && transferOptions.length > 0 && data) {
        const selectedTransfersdata = data.transfers
          .map((transfer) =>
            transferOptions.find((option) => option.value === transfer.uuid)
          )
          .filter((profile) => profile !== undefined);

        if (selectedTransfersdata) {
          handleTransferChange(selectedTransfersdata);
          // setRows([{ id: 1, chargeType: 'Private', selectedVehicle: result.vehicle[0].uuid }]);
        } else {
          // console.warn('Default vehicle not found in options');
        }
      ...
      if (result !== null) {
        const selectedRateProfiles = selRateProOptions.find(
          (option) =>
            option.value === result.vehicleTypeTransferData.rateProfile
        );

        setFormData({
          ...formData,
          status: result.status,
          childagelower: data.lowerAge,
          supplieruuid: data.supplier.uuid,
          childageupper: data.upperAge,
          applicableDays: result ? data.applicableDays : [],
          remark: data.remark,
          rateType: data.rateType,
          releasePeriodHours: data.releasePeriodHours,
          emergencyNo: data.emergencyNo,
          currencyuuid: data.currency.uuid,
          rateProfile: selectedRateProfiles,
          image: data.image,
        });
        if (result.chargeAppliedType === "Private") {
          const {
            driverTip,
            guideCost,
            parkingFee,
          ...
          } = result.vehicleTypeTransferData;

          document.querySelector(`input[name="txt_vehicle_rate_1"]`).value =
            ratePerVehicle || 0;
          document.querySelector(`input[name="parkingfee_1"]`).value =
            parkingFee || 0;
          document.querySelector(`input[name="drivertips_1"]`).value =
            driverTip || 0;
          document.querySelector(`input[name="repcost_1"]`).value =
            representativeCost || 0;
          document.querySelector(`input[name="guidecost_1"]`).value =
            guideCost || 0;
          document.querySelector(`input[name="entrancefees_1"]`).value =
            touristicEntranceFee || 0;
        } else {
          const updatedAdultRows = result.vehicleTypeTransferData.adult.map(
            (item, index) => ({
              id: index + 1,
              adultFrom: item.adultfrom,
              adultTo: item.adultto,
              adultRate: item.adultrateperpax,
              row: 1,
            })
          );
          setAdultRows(updatedAdultRows);
          const updatedChildRows = result.vehicleTypeTransferData.child.map(
            (item, index) => ({
              id: index + 1,
              childFrom: selAgeOptions.filter(
                (option) => item.childfrom === option.value
              )[0],
              childTo: selAgeOptions.filter(
                (option) => item.childto === option.value
              )[0],
              childRate: item.childrate.toString(),
              row: 1,
              childChargeType: item.childChargeType,
            })
          );
          setChildRows(updatedChildRows);
        }
      } else {
        navigateOnRefresh(
          Constants.URLConstants.CONTRACTSTRANSFERSRATESSEARCHTRANFERLIST
  ...
  const getTransfers = async () => {
    try {
      ...
      if (response.data.statusCode === 200) {
        ...
        const options = transfers.map((trans) => ({
          value: trans.uuid,
  ...
  const getVehicleTypes = async () => {
    try {
      setLoading(true);
      const response = await getDATA(ApiRoutes.TRANSFERS.VEHICLE_TYPE_TRANSFER_TARIFF);
      if (response.data.statusCode === 200) {
  ...
  //   setRows([...rows, { id: newRowId, chargeType: 'SIC Basis', selectedVehicle: null }]);
  //   setAdultRows([...adultRows, { id: adultRows.length + 1, row: newRowId, adultFrom: 1, adultTo: '', adultRate: '' }]);
  //   setChildRows([...childRows, { id: childRows.length + 1, row: newRowId, childFrom: '', childTo: '', childRate: '' ,childChargeType: 'amount'}]);
  // };
  // const removeLastRow = () => {
  ...
  const transformData = (dataa) => {
    return new Promise((resolve, reject) => {
      try {
        const transformedData = dataa
          .map((item) => {
            const {
              chargeAppliedType,
              vehicleuuid,
  ...
  const handleSubmit = async (e) => {
    ...
    if (Array.isArray(formData.rateProfile)) {
      formData.rateProfile = formData.rateProfile.map((rate) => rate.label);
    } else {
      formData.rateProfile = [formData.rateProfile.label];
    }
    ...
    if (isSuccessfull) {
      try {
        ...
        if (response.data.statusCode === 200) {
          SuccessApiToast("Transfer Tariff Rate Updated Successfully");
          navigateOnRefresh(
            Constants.URLConstants.CONTRACTSTRANSFERSRATESSEARCHTRANFERLIST
          );
        } else {
          SimpleAlert(
            "error",
            "Error",
            "Failed to Update Transfer Tariff Rate"
          );
        }
      } catch (error) {
                    ...
                    <div className="panel-body">
                      ...
                      <div className="row">
                        <div className="form-group col-md-3">
                          ...
                          <div>
                            ...
                            data.supplier.supplierName !== null
                        ...
                        <div className="form-group col-md-3">
                          ...
                          <div>
                            {data &&
                              data.transfers &&
                              data.transfers.map((transferdata, indexa) => (
                                <li key={indexa}>
                            ...
                            <div className="form-group col-md-3">
                              ...
                              <div className="input-group col-xs-12">
                                <MultiSelect
                                  ...
                                  value={row.selectedVehicle} // Set the default value
                      ...
                      {/* <div className="row add-vehicle-row mt-5">
                        <div className="col-md-12 text-center">
                          <span className="input-group">
                            <div id="addmore_span" className="input-group-addon add-more-vehicle" style={{marginRight:'10px'}} value="Add More" name="addmore"  onClick={addMoreRow} data-toggle="tooltip" data-placement="top" data-original-title="Add More Vehicle">
                              <i className="fa fa-plus" style={{ marginRight: '10px !important', color: 'white', float: 'right', fontSize: '10px', marginTop: '-14px', width: '131px' }}>&nbsp;&nbsp;Add More Vehicle</i>
                            </div>
                            <div id="remove_span" className="input-group-addon remove-vehicle" value="Remove" name="removebtn"  onClick={removeLastRow} data-toggle="tooltip" data-placement="top" data-original-title="Remove Vehicle" >
                              <i className="fa fa-minus" style={{ marginRight: '4px !important', float: 'right', fontSize: '10px', marginTop: '-14px', width: '131px' }}>&nbsp;&nbsp;Remove Vehicle</i>
                            </div>
                          </span>
                        </div>
                      </div> */}
                      <div className="row mt-4">
                        ...
                        <div className="form-group col-md-3">
                          <label>Image</label>
                          {formData.image ? (
                            <div className="image-preview-wrapper-tranfser">
                              <div className="image-container-tranfser">
                                <img
                                  src={formData.image}
                                  alt="Uploaded"
                                  className="image-preview-tranfser"
                                />
                                <span
                                  className="remove-icon-tranfser"
...
export default connect(mapStateToProps)(ContractsEditTransfersRateslist);
